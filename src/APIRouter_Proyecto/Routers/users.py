...
@router.get("/", summary="ver lista de usuario", response_model=list[User])
async def ver_usuario():
    return user_db


@router.get("/{user_id}", summary="ver usuario por id", response_model=User,
...
@router.put("/{user_id}", summary="Actualizar usuario", response_model=User)
...
@router.delete("/{user_id}", summary="Borrar usuario", response_model=DeleteResponse)
